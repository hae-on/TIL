...
```
country가 USA인 사람이 없어서 오류가 발생합니다.
```
...
```
KOR인 감독이 7명이기 때문에 출력하지 못합니다.
get는 일부 데이터만 조회 가능합니다.
```
...
```
get은 일부 데이터를 조회할 때 사용할 수 있습니다. 따라서 PK 값을 이용할 때 사용합니다.
filter는 다수의 데이터를 조회할 떄 사용할 수 있습니다. 여러개의 값이 필요할 때 사용합니다.
```
...
```python
director = Director.objects.filter(name='김철수')
director.delete()
```
